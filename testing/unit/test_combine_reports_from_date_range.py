...
class TestCombineReportsFromDateRange(unittest.TestCase):
    ...
    def test_render_results_with_no_results(self):
        ...
        expected = "\n%s\n" * 3 % (test_table.render(), category_table.render(), missing_table.render())  
    ...
    def test_main_with_local(self):
        device = 'device1'
        files_list = ['report_someotherdevice_2019-05-29T002333+0000.md.json', 
            'report_' + device + '_2019-05-29T002333+0000.md.json',
            'report_' + device + '_2020-05-29T002333+0000.md.json', #only the following 2 should be processed
            'report_' + device + '_2020-05-29T092333+0000.md.json'
        ]
        os.listdir = MagicMock(return_value=files_list)
        def custom_open(report, mode=None):
            if 'report_' + device + '_2020-05-29T002333+0000.md.json' in report:
                mock_name = "report_1.json"
            elif 'report_' + device + '_2020-05-29T092333+0000.md.json' in report:
