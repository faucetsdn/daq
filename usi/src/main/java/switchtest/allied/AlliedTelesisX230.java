...
public class AlliedTelesisX230 extends SwitchController {
    private static final String[] powerExpected = {"dev_interface", "admin", "pri", "oper", "power", "device", "dev_class", "max"};
    private static final String[] showPowerExpected = {"Interface", "Admin", "Pri", "Oper", "Power", "Device", "Class", "Max"};
    ...
    private static final Map<Pattern, String> interfaceProcessMap = Map.of(Pattern.compile("Link is (\\w+)"), "link",
            Pattern.compile("current duplex (\\w+)"), "duplex", Pattern.compile("current speed (\\w+)"), "speed");
    ...
    private Power buildPowerResponse(Map<String, String> powerMap) {
        ...
        try {
            ...
            System.out.println("Could not parse float: " + powerMap.get("max") + " or " + powerMap.get("power"));
        }
